...
async function processStudent(student) {
  try {
    ...
    if (fetchError && fetchError.code !== "PGRST116") {
      ...
      console.error(
        `Error fetching student with contestdojo_id ${student.contestdojo_id}:`,
        fetchError,
      );
      return;
    }
    ...
    if (existingStudent) {
      ...
      if (updateError) {
        console.error(
          `Error updating student ${student.contestdojo_id}:`,
          updateError,
        );
      } else {
        console.log(
          `Updated student with contestdojo_id ${student.contestdojo_id}`,
        );
      }
    } else {
      ...
      if (insertError) {
        console.error(
          `Error inserting new student ${student.contestdojo_id}:`,
          insertError,
        );
      } else {
        console.log(
          `Inserted new student with contestdojo_id ${student.contestdojo_id}`,
        );
      }
    }
  } catch (error) {
...
async function processTeam(team) {
  try {
    ...
    if (fetchError && fetchError.code !== "PGRST116") {
      console.error(
        `Error fetching team with contestdojo_id ${team.contestdojo_id}:`,
        fetchError,
      );
      return;
    }
    ...
    if (existingTeam) {
      ...
      if (updateError) {
        console.error(
          `Error updating team ${team.contestdojo_id}:`,
          updateError,
        );
      } else {
      ...
      if (insertError) {
        console.error(
          `Error inserting new team ${team.contestdojo_id}:`,
          insertError,
        );
      } else {
        console.log(
          `Inserted new team with contestdojo_id ${team.contestdojo_id}`,
        );
      }
    }
  } catch (error) {
...
async function processStudentEvent(student) {
  try {
    ...
    if (studentError) {
      console.error(
        `Error fetching student_id for ${student.contestdojo_id}:`,
        studentError,
      );
      return;
    }
    ...
    if (student.team_contestdojo_id) {
      ...
      if (teamError) {
        console.error(
          `Error fetching team_id for ${student.team_contestdojo_id}:`,
          teamError,
        );
      } else {
    ...
    if (fetchError && fetchError.code !== "PGRST116") {
      console.error(
        `Error fetching student_event for ${student.contestdojo_id}:`,
        fetchError,
      );
      return;
    }
    ...
    if (existingEntry) {
      ...
      if (updateError) {
        console.error(
          `Error updating student_event for ${student.contestdojo_id}:`,
          updateError,
        );
      } else {
        console.log(
          `Updated student_event for contestdojo_id ${student.contestdojo_id}`,
        );
      }
    } else {
      // Insert new entry
      ...
      if (insertError) {
        console.error(
          `Error inserting student_event for ${student.contestdojo_id}:`,
          insertError,
        );
      } else {
        console.log(
          `Inserted new student_event for contestdojo_id ${student.contestdojo_id}`,
        );
      }
    }
  } catch (error) {
    console.error(
      `Error processing student_event for ${student.contestdojo_id}:`,
      error,
    );
  }
}
