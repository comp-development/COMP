...
export async function fetchTestTakers(event_id: number, test_id: number) {
  const { data, error } = await supabase
    .from("test_takers")
    .select(
      "test_taker_id, student_id, start_time, end_time, students(first_name, last_name)"
    )
    .eq("test_id", test_id);
...
export async function fetchStudentEventsByIds(
  ...
  for (let i = 0; i < validIds.length; i += CHUNK_SIZE) {
    ...
    const { data, error } = await supabase
      .from("student_events")
      .select(
        "student_event_id, student_id, team_id, front_id, teams(team_name, front_id)"
      )
      .in("student_id", chunk)
...
export async function fetchStudentProblemTable(
  ...
  const { registration, testTaker, answers } = await fetchStudentProblemAnswers(
    test_id,
    student_event_id
  );
  ...
  const gradedMap = new Map<number, { points: number }>();
  let totalPoints = 0;
...
//   const studentIds = regs.map((r) => r.student_id).filter((id): id is string => id != null);
...
//       start_time,
//       end_time,
//       student:student_id (        -- join to students on student_id
//         first_name,
//         last_name
//       ),
//       test_answers (              -- join to test_answers on test_taker_id
//         test_answer_id,
//         answer_latex,
//         last_updated,
//         test_problem_id
...
export async function fetchTeamProblemTable(
    ...
    // 2) who’s on this team/event?
    const { data: regs, error: regsErr } = await supabase
      .from('student_events')
      .select('student_event_id, student_id, front_id')
      .eq('team_id', team_id)
      .eq('event_id', event_id)
    if (regsErr) throw regsErr
    if (!regs || regs.length === 0) return { problems, rows: [] }
  
    const studentIds = regs
      .map((r) => r.student_id)
      .filter((id): id is number => id != null)
  
    // 3) batch in all takers + their names + raw answers
    const { data: takers, error: takersErr } = await supabase
      ...
      .eq('test_id', test_id)
      .in('student_id', studentIds)
    if (takersErr) throw takersErr
  
    const takerIds = takers.map((t) => t.test_taker_id)
  
    // 4) …and batch in all graded answers for those takers
    const { data: graded, error: gradedErr } = await supabase
      .from('graded_test_answers')
      .select('test_taker_id, test_problem_id, points')
      .eq('test_id', test_id)
      .in('test_taker_id', takerIds)
    if (gradedErr) throw gradedErr
  
    // build a quick lookup of graded answers by taker
    const gradedByTaker = new Map<number, { test_problem_id: number; points: number }[]>()
    graded.forEach((g) => {
      const arr = gradedByTaker.get(g.test_taker_id) || []
      arr.push(g)
      gradedByTaker.set(g.test_taker_id, arr)
    })
  
    // assemble one row per registration
    const rows = regs
      .map((reg) => {
        const taker = takers.find((t) => t.student_id === reg.student_id)
        if (!taker) return null
  
        const startTs = new Date(taker.start_time).getTime()
  
        // map raw answers for fast lookup
        const ansMap = new Map<number, { answer_latex: string; last_updated: string }>()
        taker.test_answers.forEach((a) => ansMap.set(a.test_problem_id, a))
  
        // map graded points per problem
        const gArr = gradedByTaker.get(taker.test_taker_id) || []
        const gradedMap = new Map<number, number>(gArr.map((g) => [g.test_problem_id, g.points]))
        const totalPoints = gArr.reduce((sum, g) => sum + g.points, 0)
  
        const row: any = {
          student_event_id: reg.student_event_id,
          front_id:         reg.front_id,
          fullName:         `${taker.student.first_name} ${taker.student.last_name}`,
          startTime:        new Date(taker.start_time)
                              .toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
          totalPoints
        }
  
        // for each problem, stash latex, relative minutes, real‐time and points
        for (const p of problems) {
          const a = ansMap.get(p.test_problem_id)
          row[`p${p.problem_number}_latex`]  = a?.answer_latex ?? ''
          if (a) {
            const relMin = (new Date(a.last_updated).getTime() - startTs) / 60000
            row[`p${p.problem_number}_min`]  = relMin.toFixed(2)
            row[`p${p.problem_number}_rt`]   = new Date(a.last_updated)
                                                 .toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
          } else {
            row[`p${p.problem_number}_min`]  = ''
            row[`p${p.problem_number}_rt`]   = ''
          }
          // add graded points column
        //   row[`p${p.problem_number}_points`] = gradedMap.get(p.test_problem_id) ?? 0
        }
