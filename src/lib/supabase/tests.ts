...
export async function getTestFromComposeId(compose_test_id, detailed = false, customSelect = "*") {
  const { data, error } = await supabase
    .from(detailed ? "tests_detailed" : "tests")
    .select(customSelect)
...
export async function upsertIntoBucket(host_id: number, problemImages: Array<{ path: string, base64: string }>, bucketName = "problem-images") {
  try {
    // // Check if bucket exists
    // const { data: buckets, error: listError } = await supabase.storage.listBuckets();
    // if (listError) throw listError;
    ...
    for (const imageData of problemImages) {
      try {
        ...
        console.error(`Failed to upload ${imageData.path}:`, error.message);
        continue;
      }
    }
