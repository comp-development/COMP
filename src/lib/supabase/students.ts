 ...
 * 
 * @param select string
 * @returns list of student events
 */
// export async function getStudentEvents(event_id, select:string="*") {
 ...
 */
export async function getStudents(options: ProblemSelectRequest = {}) {
	let {
		customSelect = "*",
		customOrder = null,
		customEq = {  },
	...
	let selectQuery = supabase.from("students").select(customSelect);
	if (customOrder) {
