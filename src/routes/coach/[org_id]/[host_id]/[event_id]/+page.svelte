<script lang="ts">
  ...
  import { Button, ButtonGroup, Label, Modal } from "flowbite-svelte";
  ...
  import { supabase, type Unpacked } from "$lib/supabaseClient";
  import InfoToolTip from "$lib/components/InfoToolTip.svelte";
  import { debug_log } from "$lib/logger";

  let loading = $state(true);
  let coach: any = $state();
  let organizationDetails: CoachEventOrg | null = $state(null);
  let event_details: Tables<"events"> | null = $state(null);
  const event_id = parseInt($page.params.event_id);
  const org_id = parseInt($page.params.org_id);
  let draggedMember: Unpacked<
    Unpacked<CoachEventOrg["teams"]>["members"]
  > | null = null;
  let sourceTeamId: string | null = null;

  type Member = Unpacked<Unpacked<CoachEventOrg["teams"]>["members"]>;
  let studentsWithoutTeams: Member[] = $state([]);
  ...
  async function handleDrop(event: DragEvent, targetTeam: Unpacked<CoachEventOrg["teams"]> | null) {
    try {
      event.preventDefault();
      if (event.currentTarget instanceof HTMLElement) {
        event.currentTarget.style.backgroundColor = "";
      }
      console.log("target team", targetTeam, sourceTeamId);

      // Early return if dragging to same team
      if (sourceTeamId && targetTeam?.front_id === sourceTeamId) return;
      ...
      if (targetTeam === null) {
        ...
        organizationDetails!.teams = organizationDetails!.teams.map((team) => {
          if (team.front_id === sourceTeamId) {
            team.members = team.members.filter(
              (m) => m.user.student_id !== draggedMember!.user.student_id,
            );
          }
          return team;
        });
        ...
        toast.success(
          `Moving ${draggedMember!.user.first_name} to unassigned students`,
        );
        ...
        organizationDetails!.teams = organizationDetails!.teams.map((team) => {
          ...
          if (team.front_id === targetTeam.front_id) {
            team.members.push(draggedMember!);
          }
          return team;
        });
        organizationDetails = organizationDetails;
        ...
        toast.success(
          `Moving ${draggedMember.user.first_name} to team ${targetTeam.team_name}`,
        );
      } else {
  ...
  async function handleChangeTeam(newTeamData: any) {
    try {
      if (editingTeamId) {
        organizationDetails!.teams = organizationDetails!.teams.map((team) => {
          if (team.front_id === newTeamData.front_id) {
            // team.members.push(draggedMember);
            team.team_name = newTeamData.team_name;
          }
          return team;
        });
      } else {
        organizationDetails!.teams.push({ ...newTeamData, members: [] });
      }
      organizationDetails = organizationDetails;
  ...
  async function handleDeleteTeam() {
    try {
      const teamToDelete = organizationDetails!.teams.find(
        (t) => t.front_id === deleteTeamId,
      );

      if (teamToDelete?.members && teamToDelete.members.length > 0) {
        studentsWithoutTeams = [
          ...studentsWithoutTeams,
          ...teamToDelete.members,
        ];
      }

      await deleteTeam(deleteTeamId!);

      organizationDetails!.teams = organizationDetails!.teams.filter(
        (t: any) => t.front_id !== deleteTeamId,
      );
      organizationDetails = organizationDetails;
  ...
  async function handleDeleteStudentTeam(e, student) {
    try {
      ...
      organizationDetails!.teams = organizationDetails!.teams.map(
        (team: any) => {
          return {
            ...team,
            teamMembers: team.teamMembers.filter(
...
<div class="modalExterior">
  <Modal bind:open={isTeamModalOpen} size="md" autoclose={false}>
    ...
    <TeamForm
      ...
      {org_id}
      team={editingTeamId
        ? organizationDetails!.teams.find((t) => t.front_id === editingTeamId)
        : null}
