...
export const POST: RequestHandler = async (request: RequestEvent) => {
  ...
  if (refund_id == null && refunded_tickets == null) {
    return new Response("missing refund_id and number of refunded_tickets", {
      status: 400,
    });
  }
  ...
  if (refund_id == null && refunded_tickets != null) {
    ...
    const { data: existing_requests, error: existing_requests_error } =
      await adminSupabase
    ...
    if (existing_requests && existing_requests.length > 0) {
      return new Response(
        "Cannot grant new refunds until all existing pending refund requests are addressed.",
        { status: 400 }
      );
    }
    ...
    if (ticket.org_id !== null) {
      ...
      console.log(total_active_tickets - total_refunded_tickets - used_tickets);
      console.log(refunded_tickets);
    ...
    // if this is an student event, make sure they have enough spare tickets just for surity, but should work!
      ...
      const { data: already_refunded_tickets, error: refunded_tickets_error } =
        await adminSupabase
          .from("refund_requests")
          .select("quantity, ticket_orders!inner(id, student_id, event_id)") // Select id and student_id from ticket_orders
          .in("refund_status", ["PENDING", "APPROVED"])
    ...
    if (ticket.ticket_service == "eventbrite") {
      ...
      // get number of refunds approved for this ticket
      const { data: approved_requests, error: existing_requests_error } =
        await adminSupabase
          .from("refund_requests")
          .select("*")
          .eq("ticket_id", ticket_id)
      ...
      const approved_refunded_tickets: number =
        approved_requests?.reduce(
          (sum, order) => sum + (order.quantity || 0),
          0
        ) || 0;

      const total_refunded = approved_refunded_tickets + refunded_tickets;
      const active_tickets_in_orders = ticket.quantity - total_refunded;

      // as long as we have more than comp thinks we have, ok to approve refund
      console.log("approved_refunded_tickets", approved_refunded_tickets);
      console.log("total_refunded", total_refunded);
      console.log("active_tickets_in_orders", active_tickets_in_orders);

      // makes sure they refunded the correct amount in eventbrite before comp.
      const correct_amount =
        eventbriteData.costs.base_price.value <=
        event.ticket_price_cents * active_tickets_in_orders;

      // if its not refunded on eventbrite,
      // the status will be placed with the
      if (!correct_amount) {
        console.log("eventbriteData", eventbriteData);
        return new Response(
          "Event is not yet refunded properly on Eventbrite portal. Please login to the portal to refund it sufficient tickets first!",
          { status: 400 }
        );
      }
      ...
      try {
        ...
        throw new Response(`Stripe refund failed: ${stripeError.message}`, {
          status: 400,
        });
      }
    } else {
    ...
    const { error: updateError } = await adminSupabase
      ...
      .insert({
        ...
        request_reason: "This issue was initiated by the event host",
  ...
  if (refund_id != null) {
    ...
    if (refund.quantity < refunded_tickets) {
      return new Response(
        "refund request quantity is greater than number of tickets",
        { status: 400 }
      );
    }
    ...
    if (status === "APPROVED") {
      ...
      if (ticket.student_id !== null) {
        ...
        if (deleteRegError) {
          ...
          return new Response(
            "Failed to delete student-event entry: " + deleteRegError.message,
            { status: 400 }
          );
        }
        // TODO, delete an emptyteam as well
