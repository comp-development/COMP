<!-- <script lang="ts">
    ...
    (async () => {
      ...
      // grab information about this student and event
      // make sure that the student has a ticket for this event
      // make sure that the student has not already been assigned to a team

      // if they are in a team, they have to leave the team first
      // if they are not in a team, then we can proceed


      // If there is no stored transaction, redirect them back to the event page
      ...
      {#if team || org_event}
        <div class="team_info">
          ...
          {:else if team && ticket_order?.ticket_service == "stripe"}
            <div class="flex">
              <Button
                href={`/student/${$page.params.host_id}/${$page.params.event_id}/refund_request`}
                pill>Request Refund</Button
              >
            </div>
            <br />
