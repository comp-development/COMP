<script lang="ts">
  ...
  async function syncProblems() {
    try {
      ...
      let problems_sanitized = problems_data?.map((obj) => {
        return {
          problem_id: obj.id,
          problem_latex: obj.problem_latex,
          answer_late: obj.answer_latex,
          solution_latex: obj.solution_latex,
        };
      });
...
{:else if loggedIn}
  ...
  {/if}
{:else}
  <p>Please put in your COMPOSE email and password.</p>
  <br />
