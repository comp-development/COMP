...
function debug() {
	return {
		...
		script: ({ content, filename }) => {
				...
				if (!filename.includes("logger.ts") && filename.includes("src") && !filename.includes("node_modules")) {
					...
					while (true) {
						...
						const end = find_matching_paren_index(content.substr(start + target.length));
						if (end == -1) {
							out += content;
							break;
						}
						...
						const lines = (content.substr(0, start + target.length + end).match(/\n/g) || '').length;
						line_count += lines;
